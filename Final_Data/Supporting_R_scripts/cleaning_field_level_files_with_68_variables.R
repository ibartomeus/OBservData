...
# 1) Specific test for field level files with 68 variables. Only those files without error
# will be used in the next steps.
# 2) Extract field level data (with 68 variables) without errors
...
# extract_template_i_68: function that reads field_level_data files with 68 variables

extract_template_i_68 <- function(file_name){
  field_level_i <- read_csv(file_name,
                            col_types = cols(
                              study_id = col_character(),
                              study_id2 = col_character(),
                              ...
                              taxa_recorded  = col_character(),
                              use_visits_or_abundance  = col_character(),
...
report <- capture_output_lines({
  test_file("testthat/test-format-field_level-68.R", reporter = "summary")
})
...
if(length(list_files_field_level_68)>0){
  ...
  for (i in seq(length(list_files_field_level_68))) {
