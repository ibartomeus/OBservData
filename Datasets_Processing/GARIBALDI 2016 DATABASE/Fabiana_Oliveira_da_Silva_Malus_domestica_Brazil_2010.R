...
# Malus_domestica_Brazil_2010

# Load data
data_raw <- read_excel("Bahia_apple/Metanalise_tablePOLINFRUT_EDIT_OBSERV.xls")
data.coordinates <- read.xlsx("Bahia_apple/Coordenadas_Maçã_Graus_decimais.xlsx")
data.yield <- read_excel("Bahia_apple/PRODUÇÃO EM KG- LOTES EM QUE FOI REALIZADO TRABALHOS COM POLINIZAÇÃO (2).xls")
...
data_raw <- data_raw[grepl("2010",data_raw$Site_name,ignore.case = TRUE),]
...
data.site$study_id <- "Fabiana_Oliveira_da_Silva_Malus_domestica_Brazil_2010"
...
which(names(data_raw)=="Geotrigona sp")

data_raw_obs_ini <- data_raw[,c(3,14,53:62)] %>% 
  rename(site_id=Site_name,"Apis mellifera"=Apis_mellifera...53)

data_raw_obs_ini %>% group_by(site_id,Crop_variety) %>% count()

# Gather abundance info

data_raw_obs <- data_raw_obs_ini %>% gather(Organism_ID,abundance,c(3:12)) %>%
...
list_organisms_guild$Guild[list_organisms_guild$Organism_ID=="Geotrigona sp"] <- "other_wild_bees"
list_organisms_guild$Guild[list_organisms_guild$Organism_ID=="Exomalopsis analis...61"] <- "other_wild_bees" #RECHECK
list_organisms_guild$Guild[list_organisms_guild$Organism_ID=="Dialictus sp2"] <- "other_wild_bees"
...
sampling_rounds <- data_raw_obs_ini %>% group_by(site_id,Crop_variety) %>% count() %>%
  mutate(n=n/8)

data_obs_guild <- data_obs_guild %>% left_join(sampling_rounds,by="site_id")

insect_sampling <- tibble(
  study_id = "Fabiana_Oliveira_da_Silva_Malus_domestica_Brazil_2010",
  ...
  total_sampled_time = 5*8*data_obs_guild$n,
  total_sampled_flowers = NA,
  Description = "In each orchad abundance of floral visitors was analysed by sweep-netting all visitors along four pairs of trees for five minutes each, for a total of 20 minutes of sampling. Sampling was further repeated on at least two dates during the main flowering period")
...
list_organisms_guild_ob$Guild[list_organisms_guild_ob$Organism_ID=="Trigona spp.."] <- "other_wild_bees"
list_organisms_guild_ob$Guild[list_organisms_guild_ob$Organism_ID=="Xylocopa"] <- "other_wild_bees" #RECHECK
...
write_csv(insect_sampling_total, "insect_sampling_Fabiana_Oliveira_da_Silva_Malus_domestica_Brazil_2010.csv")
setwd(dir_ini)
...
data_obs_guild_2 <-  bind_rows(data_obs_guild,data_obs_guild_ob) %>% select(-Crop_variety) %>%
...
percentage_species_morphos <- 223/236
...
data_obs_guild_2_ob <-  data_obs_guild_ob %>% select(-Crop_variety) %>%
  group_by(site_id,Organism_ID,Guild) %>% summarise_all(sum,na.rm=TRUE)
...
visits_aux <- visits_aux %>% mutate(beetles=0,bumblebees=0,other_flies=0,
                                          lepidoptera=0,non_bee_hymenoptera=0,other=0,
                                          syrphids=0,humbleflies=0,total=0)
...
setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "field_level_data_Fabiana_Oliveira_da_Silva_Malus_domestica_Brazil_2010.csv")
setwd(dir_ini)
