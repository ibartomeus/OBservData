...
##########################
#Data: Szentgyorgyi_NA
##########################

library(openxlsx)

data_raw <- read_csv("Individual CSV/Szentgyorgyi_NA.csv")
data_raw$date_round1 <- openxlsx::convertToDate(data_raw$date_round1)
data_raw$year_round1 <- as.numeric(format(as.Date(data_raw$date_round1, format="%Y/%m/%d"),"%Y"))
data_raw$month_of_study <- as.numeric(format(as.Date(data_raw$date_round1, format="%Y/%m/%d"),"%m"))
...
data.site_aux <- tibble(
  ...
  sampling_year = data_raw_without_NAs$year_round1,
  field_size = NA,
...
data_raw_obs <- data_raw %>%
  select(site, names(data_raw[30:ncol(data_raw)])) %>% select(-year_round1)
...
data_transects <- data_raw_without_NAs %>% group_by(site) %>% count() %>% 
  rename(site_id=site)

data_raw_gather <- data_raw_gather %>% left_join(data_transects,by=c("site_id"))


insect_sampling <- tibble(
  study_id = "Szentgyorgyi_NA",
  site_id = data_raw_gather$site_id,
  pollinator = data_raw_gather$Organism_ID,
  guild = data_raw_gather$Guild,
  sampling_method = "transects+pantraps",
...
abundance_aux <- abundance_aux %>% mutate(bumblebees=0,lepidoptera=0,syrphids=0,
  other=0,humbleflies=0,other_wild_bees=0,total=0)
...
setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "field_level_data_Szentgyorgyi_NA.csv")
setwd(dir_ini)
