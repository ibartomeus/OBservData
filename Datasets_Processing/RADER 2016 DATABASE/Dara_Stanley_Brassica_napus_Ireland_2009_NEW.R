...
data_raw_obs <- data_raw %>%
  select(site,observation_location, names(data_raw[30:ncol(data_raw)])) %>%
  rename(site_aux=observation_location) %>%
  mutate(site_aux=paste(site," ",site_aux))
...
data_raw_gather <- data_raw_obs %>% gather(-site,-site_aux,key = "Organism_ID", value = 'Abundance', !contains("site"))
...
insect_sampling <- tibble(
  ...
  Description = paste("Specific location:",data_raw_gather$site_aux,". Each field was visited three times and there were four 100 × 2 m transects walked on each sampling visit (two on the edge and two on the centre), at a slow, steady pace (0.07-0.44 m/s).")
)

insect_sampling %>% group_by(site_id,pollinator) %>% count()
