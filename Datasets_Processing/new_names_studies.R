...
for (i in 1:length(list_files_field_level)){
  ...
  if (length(unique(data.site$crop))>1){
    crop <- "several_crops"
  }else{
  ...
  if (length(unique(data.site$country))>1){
    country <- "several_countries"
  }else{
    country <- unique(data.site$country)
    country <- str_replace(country," ","_")
  }
  
  if (length(unique(data.site$sampling_year))>1){
    year <- "several_years"
  }else{
    year <- unique(data.site$sampling_year)
...
result$author_folder[grepl("Davi L. Ramos",result$Credit,ignore.case = TRUE)] <-"Davi L. Ramos"
result$author_folder[grepl("https://snd.gu.se",result$Credit,ignore.case = TRUE)] <-"Rebecca Steward"
result$author_folder[grepl("Rachel Mallinger",result$Credit,ignore.case = TRUE)] <-"Rachel Mallinger"
result$author_folder[grepl("Sarah Cusser",result$Credit,ignore.case = TRUE)] <-"Sarah Cusser"
result$author_folder[is.na(result$author_folder)] <- result$Credit[is.na(result$author_folder)]

result$author_name <- NA
result$new_code <- NA

for (i in 1:nrow(result)){
  name <- str_split(result$author_folder,",")[i]
  name[[1]][1]
  result$author_name[i] <- name[[1]][1]
  result$author_name[i] <- str_replace_all(result$author_name[i]," ","_")
  result$author_name[i] <- str_replace_all(result$author_name[i],"[.]","")
  result$author_name[i] <- str_replace_all(result$author_name[i],"-","_")
  result$new_code[i] <- paste(result$author_name[i],
                              result$crop[i],
                              result$country[i],
                              result$year[i],sep = "_")
}
...
result$new_code[duplicated(result$new_code)]

result_final <- result %>% select(old_code,new_code)


write_csv(result_final,"new_names_studies.csv")
