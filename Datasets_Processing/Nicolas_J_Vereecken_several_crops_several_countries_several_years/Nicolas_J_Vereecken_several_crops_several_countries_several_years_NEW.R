...
data.site <- read.xlsx("Observ_TW_NEW.xlsx",
                          sheet = "field_level_data", startRow = 1)
data.site <- as_tibble(data.site)

#Load datasets: insect_sampling
data.insect <- read.xlsx("Observ_TW_NEW.xlsx",
...
#Fixing total yield format [kg/ha]:
data.site <- data.site %>% separate(total_yield,c("total_yield","units")," ")
data.site$total_yield <- str_replace_all(data.site$total_yield,",",".")

as.numeric(data.site$total_yield)
data.site$total_yield <- as.numeric(data.site$total_yield)
data.site$total_yield[!is.na(data.site$units)] <- 1000*data.site$total_yield[!is.na(data.site$units)]
