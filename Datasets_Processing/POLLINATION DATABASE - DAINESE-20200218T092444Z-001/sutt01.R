...
data.species_01 %>% group_by(Identified_to) %>% count()
percentage_species_morphos <-
  sum(data.species_01$Identified_to %in% c("species"))/nrow(data.species_01)
...
data.species_01 <- data.species_01 %>% mutate(total_sampled_area=NA,
                                              ...
                                              Description="4x(2 observatiosn, 2 sweep, and 4 pitfalls) per site") 
...
write_csv(insect_sampling, "insect_sampling_sutt01.csv")
...
abundance_aux <- data.species_01 %>% filter(sampling_method!="pitfall") %>% 
  group_by(site_id,Guild) %>% count(wt=abundance) %>% 
  spread(key=Guild, value=n)

names(abundance_aux)

# All guilds are present
...
abundace_field <- data.species_01 %>% filter(sampling_method!="pitfall") %>%
...
setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "field_level_data_sutt01.csv")
setwd(dir_ini)
