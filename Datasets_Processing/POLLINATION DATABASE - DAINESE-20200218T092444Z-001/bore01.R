...
data.site <- read.xlsx("DATASETS/Bore01_Datacollection_pollination.xlsx",
...
data.site <- data.site %>% select(-'Annual/perennial')%>%
  rename(site_id=SiteID,longitude=X,latitude=Y,sampling_year=Year,crop=Crop.species,
         management=Management) %>% mutate(study_id="bore01")
...
data.Functioning <- read.xlsx("DATASETS/Bore01_Datacollection_pollination.xlsx",
...
write_csv(insect_sampling, "insect_sampling_bore01.csv")
...
data.site$pollinator_richness[is.na(data.site$pollinator_richness)] <- 0.0
data.site$richness_estimator_method[is.na(data.site$richness_estimator_method)] <- "observed"
...
setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "field_level_data_bore01.csv")
setwd(dir_ini)
