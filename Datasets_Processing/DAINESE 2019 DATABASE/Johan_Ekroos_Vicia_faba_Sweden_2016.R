...
data.site <- read.xlsx("DATASETS/Ekro01_Datacollection_pollination.xlsx",
...
data.site$study_id <- "Johan_Ekroos_Vicia_faba_Sweden_2016"
...
data.Functioning <- read.xlsx("DATASETS/Ekro01_Datacollection_pollination.xlsx",
...
data.Functioning <- data.Functioning %>%
  rename(site_id=SiteID,sampling_year=Year.of.sampling,
         fruit_weight=`Function.(total.bean.weight)`,
         seeds_per_fruit=`Function.(average.number.of.developed.beans)`) %>%
  select(-Exclosure.treatment,-Type.of.function,-`Function.(average.bean.weight.per.pod)`)
...
data.site <- data.site %>% mutate(country="Sweden",Publication=NA,
                                  Credit="Johan Ekroos & Henrik Smith (Lund University)",
                                  email="johan.ekroos@cec.lu.se")
...
data.species_01 <- data.species_01 %>% mutate(total_sampled_area=300,
                                              total_sampled_time=NA,
                                              total_sampled_flowers=NA,
                                              Description="4 potted field bean plants per landscape; 100 m transect counts for pollinator densities.	3 sampling rounds (pollinators)") 


insect_sampling <- tibble(
  study_id = "Johan_Ekroos_Vicia_faba_Sweden_2016",
...
write_csv(insect_sampling, "insect_sampling_Johan_Ekroos_Vicia_faba_Sweden_2016.csv")
...
abundance_aux <- data.species_01 %>% filter(sampling_method=="transect counts") %>% 
...
abundance_aux <- abundance_aux %>% mutate(honeybees=0, other_wild_bees=0, syrphids=0, humbleflies=0,
                                          other_flies=0, beetles=0, non_bee_hymenoptera=0, lepidoptera=0,
                                          other=0,total=0)
...
abundace_field <- data.species_01 %>% filter(sampling_method=="transect counts") %>%
