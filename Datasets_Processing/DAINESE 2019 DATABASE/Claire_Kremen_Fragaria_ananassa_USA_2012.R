...
options(digits=14)
##########################
#Data: DAINESE, Scil01: scil01
##########################


data.site <- read.xlsx("DATASETS/Scil01_Datacollection_pollination.xlsx",
                          sheet = "SiteData", startRow = 2)
data.site <- as_tibble(data.site)

data.site$study_id <- "Claire_Kremen_Fragaria_ananassa_USA_2012"
...
data.site <- data.site %>% select(-'Annual/perennial',-Local.diversity)%>%
...
data.Functioning <- read.xlsx("DATASETS/Scil01_Datacollection_pollination.xlsx",
                                 sheet = "Functioning", startRow = 2)

data.Functioning <- as_tibble(data.Functioning)

data.Functioning <- data.Functioning %>% select(-standard.error) %>%
  spread(Exclosure.treatment, Function)

data.Functioning <- data.Functioning %>% select(-wind_and_pollinators_excluded) %>%
  rename(site_id=SiteID,
         sampling_year=Year.of.sampling,
         yield_units= Type.of.function,
         yield=open_pollinated_control,
         yield_treatments_no_pollinators=`pollinators excluded`)

data.Functioning1 <- data.Functioning %>% filter(yield_units=="mean_berry_weight")

data.site <- data.site %>% left_join(data.Functioning1, by = c("site_id","sampling_year"))

data.Functioning2 <- data.Functioning %>% filter(yield_units!="mean_berry_weight")
data.Functioning2 <- data.Functioning2 %>%
  rename(yield2_units= yield_units,
         yield2=yield,
         yield_treatments_no_pollinators2=yield_treatments_no_pollinators)

data.site <- data.site %>% left_join(data.Functioning2, by = c("site_id","sampling_year"))
...
data.site <- data.site %>% mutate(country="USA",Publication=NA,
                                  Credit="Amber Sciligo, Claire Kremen (UC Berkeley)",
                                  email="amber.sciligo@berkeley.edu")
...
percentage_species_morphos <-
  sum(data.species_01$Identified.to %in% c("species","morphospecies","morpospecies"))/nrow(data.species_01)
...
#######################################
## ANOTHER SHEET WITH ADDITIONAL INFO
...
data.species_02 <- data.species_02 %>% left_join(gild_list[,c(1,3)],by=c("Organism_ID"))
#Check NA's in guild
data.species_02 %>% filter(is.na(Guild)) %>% group_by(Organism_ID,Family) %>% count()

data.species_02 <- data.species_02 %>% filter(abundance!=0)

data.species_01 <- bind_rows(data.species_01,data.species_02)

data.species_01 <- data.species_01 %>% mutate(total_sampled_area=NA,
                                              total_sampled_time=NA,
                                              total_sampled_flowers=NA,
                                              Description="6 10 minute transects; 21 pan traps for 5 hrs	2 sampling rounds: Spring and Summer
") 

data.species_01$total_sampled_time[data.species_01$sampling_method=="2_rounds_of_6_10_min_transects"] <- 120
data.species_01$total_sampled_time[data.species_01$sampling_method=="2_rounds_21_pan_traps_5hr"] <- 300

insect_sampling <- tibble(
  study_id = "Claire_Kremen_Fragaria_ananassa_USA_2012",
...
write_csv(insect_sampling, "insect_sampling_Claire_Kremen_Fragaria_ananassa_USA_2012.csv")
...
abundance_aux <- data.species_01 %>% filter(Number.of.censuses=="2_rounds_of_6_10_min_transects") %>% 
...
abundace_field <- data.species_01 %>% filter(Number.of.censuses=="2_rounds_of_6_10_min_transects") %>%
...
setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "field_level_data_Claire_Kremen_Fragaria_ananassa_USA_2012.csv")
setwd(dir_ini)
