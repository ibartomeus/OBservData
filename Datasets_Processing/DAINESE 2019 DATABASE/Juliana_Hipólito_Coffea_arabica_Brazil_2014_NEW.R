...
options(digits=14)
##########################
#Data: DAINESE, Hipo01: Hipo02 2014
##########################


data.site <- read.xlsx("DATASETS/Hipo01_Datacollection_pollination.xlsx",
                          sheet = "SiteData", startRow = 2)
data.site <- as_tibble(data.site)

data.site$study_id="Juliana_Hipólito_Coffea_arabica_Brazil_2014"
...
data.Final <- read.xlsx("DATASETS/Hipo01_Datacollection_pollination.xlsx",
...
data.LandscapeData <- read.xlsx("DATASETS/Hipo01_Datacollection_pollination.xlsx",
...
data.species_01$Guild[data.species_01$Organism_ID=="Nannotrigona testaceicornes "] <- "other_wild_bees"
...
insect_sampling <- tibble(
  study_id = "Juliana_Hipólito_Coffea_arabica_Brazil_2014",
...
write_csv(insect_sampling, "insect_sampling_Juliana_Hipólito_Coffea_arabica_Brazil_2014.csv")
...
# There are all types but humbleflies
...
field_level_data_mod <- read_csv("DATASETS/field_level_data_Juliana_Hipólito_Coffea_arabica_Brazil_2014.csv")

# Adding latitud and longitude
lat_long_info <- read_csv("DATASETS/field_level_data_Juliana_Hipólito_Coffea_arabica_Brazil_2013.csv") %>%
  select(site_id,latitude,longitude)

# Fixing geolocalization errors
long <- parse_lon(lat_long_info$latitude)
lat <- parse_lat(lat_long_info$longitude)

lat_long_info$latitude <- lat
lat_long_info$longitude <- long

site_ids_lat_long <- field_level_data_mod %>% select(site_id) %>% left_join(lat_long_info,by="site_id")

# Adding missing coordinates (site: https://epsg.io/transform#s_srs=5534&t_srs=4326&x=241814.0000000&y=8524195.0000000)

site_ids_lat_long$latitude[site_ids_lat_long$site_id=="O07"] <- "-13.3691672"
site_ids_lat_long$longitude[site_ids_lat_long$site_id=="O07"] <- "-41.3285047"

site_ids_lat_long$latitude[site_ids_lat_long$site_id=="O13"] <- "-13.3336629"
site_ids_lat_long$longitude[site_ids_lat_long$site_id=="O13"] <- "-41.3591028"

site_ids_lat_long$latitude[site_ids_lat_long$site_id=="O15"] <- "-13.3388612"
site_ids_lat_long$longitude[site_ids_lat_long$site_id=="O15"] <- "-41.3839098"

field_level_data_mod$latitude <- site_ids_lat_long$latitude
field_level_data_mod$longitude <- site_ids_lat_long$longitude

setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data_mod, "field_level_data_Juliana_Hipólito_Coffea_arabica_Brazil_2014.csv")
setwd(dir_ini)
