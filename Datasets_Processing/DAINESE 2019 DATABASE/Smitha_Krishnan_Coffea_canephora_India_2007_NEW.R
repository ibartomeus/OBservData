...
options(digits=14)
##########################
#Data: DAINESE, krish01: krish011
##########################


data.site <- read.xlsx("DATASETS/Krish01_DataCollection_Pollination.xlsx",
...
sampling_year <- sampling %>% filter(sampling_year==2007) %>% 
...
data.Functioning <- read.xlsx("DATASETS/Krish01_Datacollection_pollination.xlsx",
...
yield_aux21 <- yield_aux2 %>% group_by(site_id) %>% summarise(sampling_year=mean(sampling_year,na.rm = T),
...
data.species_01$abundance <- round(3*data.species_01$abundance)
...
insect_sampling <- tibble(
  study_id = "Smitha_Krishnan_Coffea_canephora_India_2007",
...
write_csv(insect_sampling, "insect_sampling_Smitha_Krishnan_Coffea_canephora_India_2007.csv")
...
number_sites <- read.xlsx("DATASETS/Krish01_DataCollection_Pollination.xlsx",
                          sheet = "SiteData", startRow = 2)

number_sites <- as_tibble(number_sites)

number_sites <- number_sites %>% filter(Year==2007)
...
#total number of inflorescences = 3*n*5*6=90*n

for (i in 1:nrow(visit_aux)){
  a <- 45*90*pull(visit_aux[i,2])*pull(visit_aux[i,2])
...
setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "field_level_data_Smitha_Krishnan_Coffea_canephora_India_2007.csv")
setwd(dir_ini)
