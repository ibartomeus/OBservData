...
data.site <- data.site %>% filter(SiteID=="chac02")
...
data.Functioning <- data.Functioning %>% filter(SiteID=="chac01",Year.of.sampling==2001)
...
data.Functioning1 <- data.Functioning %>% select(-dis,Pollen.grains.ALLOGAMY,
                                                -Pollen.grains.OPEN.FLOWERS,
                                                -Pollen.grains.ALLOGAMY,
                                                -Fruit.set.for.EXCLOSURES,-Function)

data.Functioning1 <- data.Functioning1 %>% group_by(SiteID,site,Year.of.sampling) %>%
...
data.site$sampling_start_month <- 8
data.site$sampling_end_month <- 9

###########################
# Adding fruit weight & seeds_per_fruit

data.site$fruit_weight <- c(181.834,
                            152.5355,
                            160.779464285714,
                            154.298679245283)

data.site$seeds_per_fruit <- c(2.3,
                               3.45,
                               2.58928571428571,
                               2.60377358490566)
...
insect_sampling_visit <- tibble(
  study_id = "Natacha_Chacoff_Citrus_paradisi_Argentina_2001",
...
flowers.data <- read.xlsx("DATASETS/data chacoff grapefruit 2001.xlsx", sheet = "species")

#Sanity check
flowers.data[,c(2,3,5)]==insect_sampling_visit[,c(2,3,5)]
flowers.data[1,c(5)]
insect_sampling_visit[1,c(5)]
...
honeybees.data <- read.xlsx("DATASETS/Chac01_Datacollection_pollination.xlsx",
                            sheet = "StudyMetadata", startRow = 2) %>%
  filter(StudyID=="chac01",`Study.year(s)`==2001)
...
write_csv(insect_sampling, "insect_sampling_Natacha_Chacoff_Citrus_paradisi_Argentina_2001.csv")
...
abundance.data <- read.xlsx("DATASETS/Chac01_Datacollection_pollination.xlsx",
                            sheet = "StudyMetadata", startRow = 2) %>%
  filter(StudyID=="chac01",`Study.year(s)`==2001) %>%
...
#"beetles"             "honeybees"          
#[5] "non_bee_hymenoptera" "other_flies"         "other_wild_bees"

# GUILDS:honeybees, bumblebees, other wild bees, syrphids, humbleflies,
# other flies, beetles, non-bee hymenoptera, lepidoptera, and other

visitation_aux <- mutate(visitation_aux,lepidoptera=0,
                         bumblebees=0,syrphids=0, humbleflies=0,
                         other=0,total=0)
...
setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "field_level_data_Natacha_Chacoff_Citrus_paradisi_Argentina_2001.csv")
setwd(dir_ini)
