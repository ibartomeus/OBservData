...
options(digits=14)
##########################
#Data: DAINESE, Chac01: chac03 year:2002
##########################
...
data.site <- data.site %>% filter(SiteID=="chac03")
...
data.Functioning <- data.Functioning %>% filter(SiteID=="chac01",Year.of.sampling==2002)
...
data.site$fruit_weight <- c(192,
                            198.26,
                            186.96,
                            198.57)

data.site$seeds_per_fruit <- c(2.33,
                               3.94,
                               3.23,
                               3.37)
...
insect_sampling_visit <- tibble(
  study_id = "Natacha_Chacoff_Citrus_paradisi_Argentina_2002",
...
flowers.data <- read.xlsx("DATASETS/data chacoff grapefruit 2002.xlsx", sheet = "species")
...
honeybees.data <- read.xlsx("DATASETS/Chac01_Datacollection_pollination.xlsx",
                            sheet = "StudyMetadata", startRow = 2) %>%
  filter(StudyID=="chac01",`Study.year(s)`==2002)
...
write_csv(insect_sampling, "insect_sampling_Natacha_Chacoff_Citrus_paradisi_Argentina_2002.csv")
...
#"beetles"             "honeybees","lepidoptera","other",
#"non_bee_hymenoptera", "other_flies", "other_wild_bees"

# GUILDS:honeybees, bumblebees, other wild bees, syrphids, humbleflies,
# other flies, beetles, non-bee hymenoptera, lepidoptera, and other

visitation_aux <- mutate(visitation_aux,
...
setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "field_level_data_Natacha_Chacoff_Citrus_paradisi_Argentina_2002.csv")
setwd(dir_ini)
