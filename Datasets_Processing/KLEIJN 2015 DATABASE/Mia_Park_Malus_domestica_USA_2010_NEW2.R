...
data.site$study_id <- "Mia_Park_Malus_domestica_USA_2010"
...
data_raw_obs %>%  filter(Year==2010) %>% select(Site,Sampling_Date,Number_Censuses) %>%
  group_by(Site,Sampling_Date,Number_Censuses) %>%
  unique() %>% count()


data_raw_obs_year <- data_raw_obs %>%  filter(Year==2010) %>%
...
insect_sampling <- tibble(
  study_id = "Mia_Park_Malus_domestica_USA_2010",
...
write_csv(insect_sampling, "insect_sampling_Mia_Park_Malus_domestica_USA_2010.csv")
setwd(dir_ini)
...
setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "field_level_data_Mia_Park_Malus_domestica_USA_2010.csv")
setwd(dir_ini)
