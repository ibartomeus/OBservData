...
##########################
#Data: 58_MacKenzie_AverillExtractedFromPaper
##########################
...
data.site <- read.xlsx("58_59_MacKenzie_AverillExtractedFromPaper/MacKenzie&Averill1995.xlsx",
                       sheet = "Sheet1",startRow = 51)

data.site <- data.site[1:8,c(1,8,12)]
...
data.site$study_id <- "David_Kleijn_Vaccinium_macrocarpon_USA_1990"
...
data.insect <- read.xlsx("58_59_MacKenzie_AverillExtractedFromPaper/MacKenzie&Averill1995.xlsx",
                         sheet = "Sheet1",startRow = 2)

#Select 1990 data

data_raw_obs <- data.insect[1:42,1:9]
data_raw_obs <- as_tibble(data_raw_obs)

data_raw_obs <- data_raw_obs %>% gather("site_id","abundance",
                       c(C1,C2,C3,A1,A2,A3,N1,N2))

# Filter NAs

data_raw_obs <- data_raw_obs %>% filter(!is.na(abundance)) %>% rename(Organism_ID=X1)
...
insect_sampling <- tibble(
  study_id = "David_Kleijn_Vaccinium_macrocarpon_USA_1990",
  ...
  total_sampled_time = 3*15, #netting+observations
...
write_csv(insect_sampling, "insect_sampling_David_Kleijn_Vaccinium_macrocarpon_USA_1990.csv")
setwd(dir_ini)
...
setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "field_level_data_David_Kleijn_Vaccinium_macrocarpon_USA_1990.csv")
setwd(dir_ini)
