...
data_raw_obs <- read.xlsx("42_44_MiaParkApple_2009_2010_2011/Park_Temporal variation meta-data template v2020.xlsx",
...
# Check number of censuses among sites


sampling <- data_raw_obs %>%  filter(Year==2011) %>%
  select(Site,Sampling_Method,Number_Censuses,Floral.abundance) %>%
  unique() %>%
  group_by(Site,Sampling_Method) %>%
  summarise(Number_Censuses=sum(Number_Censuses),
            Floral.abundance=sum(Floral.abundance)) %>%
  mutate(total_sampled_area=100*Number_Censuses,
         total_sampled_time=Number_Censuses*15,
         total_sampled_flowers=Floral.abundance) %>%
  rename(site_id=Site)

data.site$site_id[!data.site$site_id %in% sampling$site_id]

data_raw_obs$Species[data_raw_obs$Species=="Andrena morrisonella "] <- "Andrena morrisonella"

data_raw_obs_year <- data_raw_obs %>%  filter(Year==2011) %>%
  select(Site,Species,Abundance) %>%
  rename(site_id=Site,Organism_ID=Species,abundance=Abundance) %>%
  group_by(site_id,Organism_ID) %>%
  summarise(abundance=sum(abundance))

data_raw_obs_year %>% group_by(site_id,Organism_ID) %>% count() %>% filter(n>1)

data_raw_obs_year <- data_raw_obs_year %>% left_join(sampling,by="site_id")
...
data_obs_guild <- data_raw_obs_year %>% left_join(list_organisms_guild,
                                                  by = c("site_id","Organism_ID"))
...
data_obs_guild_2 <-  data_obs_guild %>% select(-Sampling_Method) %>%
...
data_raw_obs %>% filter(Year==2011) %>% select(Site) %>% unique() %>% pull()
field_level_data %>% filter(is.na(observed_pollinator_richness)) %>% select(site_id)
