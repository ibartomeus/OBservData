...
data.site_ca <- read_csv("datasets/watermelon_ca_dataset.csv")
data.site_fl <- read_csv("datasets/watermelon_fl_dataset.csv") #%>% rename(wild_bee=wild_bees)  
  
data.site <- bind_rows(data.site_ca,data.site_fl)
...
# New study_id
data.site$study_id <- paste0("James_Reilly_Citrullus_lanatus_USA_",data.site$year)
...
write_csv(field_level_data, "field_level_data_James_Reilly_Citrullus_lanatus_USA_2013-14-15.csv")
setwd(dir_ini)
