...
##########################
#Data: 45_RachaelWinfree_Apple_2004
##########################

data_raw <- read.xlsx("Processing_files/Datasets_processing/KLEIJN 2015 DATABASE/45_RachaelWinfree_Apple_2004/STEP4_winfree_apple.xlsx",
...
data_raw %>% group_by(latitude,longitude) %>% count()
...
data.site <- data_raw %>% filter(latitude!="NULL") %>% select(latitude,longitude) %>%
  group_by(latitude,longitude) %>% count() %>% select(-n)
# We add data site ID
data.site$site_id <-  seq.int(nrow(data.site))

data.site$study_id <- "Rachael_Winfree_Malus_Domestica_USA_2004"
...
data.site$Credit <- "Rachael Winfree"
data.site$Email_contact <- "rwinfree@rutgers.edu"

# Add sampling months

site_coordinates <- data.site %>% select(site_id,latitude,longitude)
data_raw <- data_raw %>% left_join(site_coordinates, by = c("latitude","longitude"))
...
data_raw_obs <- data_raw %>%
  select(genus_species,latitude,longitude) %>% rename(Organism_ID=genus_species)%>%
  left_join(site_coordinates, by = c("latitude","longitude")) %>%
...
insect_sampling <- tibble(
  study_id = "Rachael_Winfree_Malus_Domestica_USA_2004",
...
write_csv(insect_sampling, "Processing_files/Datasets_storage/insect_sampling_Rachael_Winfree_Malus_Domestica_USA_2004.csv")
#setwd(dir_ini)
...
# Remove unknown site
abundance_aux <- abundance_aux %>% filter(!is.na(site_id))
...
# Remove unknown site
abundace_field <- abundace_field %>% filter(!is.na(site_id))
...
#setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "Processing_files/Datasets_storage/field_level_data_Rachael_Winfree_Malus_Domestica_USA_2004.csv")
#setwd(dir_ini)
