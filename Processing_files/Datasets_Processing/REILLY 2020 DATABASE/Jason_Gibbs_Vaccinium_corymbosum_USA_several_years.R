...
data.site <- data.site_mi_bc_or %>% filter(state=="mi") #bind_rows(data.site_fl,data.site_mi_bc_or)
...
# New study_id
data.site$study_id <- paste0("Jason_Gibbs_Vaccinium_corymbosum_USA_",data.site$year)
...
write_csv(field_level_data, "Processing_files/Datasets_storage/field_level_data_Jason_Gibbs_Vaccinium_corymbosum_USA_2013-14-15.csv")
#setwd(dir_ini)
