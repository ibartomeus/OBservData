...
# We assume that the first 15 fields correspond to Groo01
#data.LandscapeData <- unique(data.LandscapeData)

data.LandscapeData <- data.LandscapeData[1:15,c(1,4)]

data.site <- data.site %>% left_join(data.LandscapeData, by = c("site_id"))
data.site <- data.site %>% mutate(country="Netherlands",Publication=NA,
...
data.species_01 <- data.species %>% filter(sampling_year==2013,site_id %in% data.site$site_id)
...
data.species_01 %>% select(-Identified.to,-X6)
...
insect_sampling <- tibble(
  study_id = "David_Kleijn_Malus_domestica_Netherlands_2013",
...
write_csv(insect_sampling, "Processing_files/Datasets_storage/insect_sampling_David_Kleijn_Malus_domestica_Netherlands_2013.csv")
...
field_level_data_mod <- read_csv("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/field_level_data_David_Kleijn_Malus_domestica_Netherlands_2013.csv")
field_level_data$variety <- field_level_data_mod$variety
field_level_data$yield_units <- field_level_data_mod$yield_units

# setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "Processing_files/Datasets_storage/field_level_data_David_Kleijn_Malus_domestica_Netherlands_2013.csv")
# setwd(dir_ini)
