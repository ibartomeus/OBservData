...
options(digits=14)
##########################
#Data: DAINESE, Grab01: conn01
##########################


data.site <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Grab01_Datacollection_pollination.xlsx",
                          sheet = "SiteData", startRow = 2)
data.site <- as_tibble(data.site)

data.site <- data.site %>% filter(`StudyID?`=="conn01")
...
data.Final <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Grab01_Datacollection_pollination.xlsx",
                                 sheet = "Final Ecosystem Services", startRow = 2)

data.Final <- as_tibble(data.Final)

data.Final <- data.Final %>% filter(StudyID=="conn01") %>% select(-StudyID)
...
data.Final <- data.Final %>% rename(yield=open,
  ...
  select(-Unit,-X6) %>% mutate(yield_units="grams (mean fruit weight for 10 fruits per field)")
...
data.site$field_size <- NA #50m2 experimental plots in Conn2

data.site <- data.site %>% mutate(country="USA",Publication="10.1016/j.agee.2015.05.004",
                                  Credit="Heather Grab, USDA NASS",
...
data.species <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Grab01_Datacollection_pollination.xlsx",
...
data.species_01 <- data.species_01 %>% mutate(total_sampled_area=NA,
                                              ...
                                              Description="5 pan traps per site 2 m apart.Temporal replicates: 2x per season")


insect_sampling <- tibble(
  study_id = "Heather_Lee_Grab_Fragaria_ananassa_USA_2012",
...
write_csv(insect_sampling, "Processing_files/Datasets_storage/insect_sampling_Heather_Lee_Grab_Fragaria_ananassa_USA_2012.csv")
...
# setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "Processing_files/Datasets_storage/field_level_data_Heather_Lee_Grab_Fragaria_ananassa_USA_2012.csv")
# setwd(dir_ini)
...
field_level_data <- read_csv("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/field_level_data_Heather_Lee_Grab_Fragaria_ananassa_USA_2012_ed.csv")
