...
data.site <- data.site %>% mutate(country="Brazil",Publication="10.1126/science.1230200",
                                  Credit="Breno M. Freitas",email="freitas@ufc.br,antonniodiego@hotmail.com")
...
data.species_03 <- data.species %>% filter(sampling_year==2012,site_id %in% data.site$site_id)
...
data.species_03 <- data.species_03 %>% mutate(total_sampled_area=Number.of.censuses*150,
                                              ...
                                              Description="6 subplots per site,	150 m, 10 sampling rounds in one season")


insect_sampling <- tibble(
  study_id = "Breno_M_Freitas_Anacardium_occidentale_Brazil_2012",
...
write_csv(insect_sampling, "Processing_files/Datasets_storage/insect_sampling_Breno_M_Freitas_Anacardium_occidentale_Brazil_2012.csv")
...
data.species_03 %>% group_by(Guild) %>% count(wt=abundance)

# There are only honeybees,lepidoptera, non_bee_himenoptera,other_flies,other_wild_bees

abundance_aux <- abundance_aux %>% mutate(bumblebees=0,syrphids=0,humbleflies=0,
                                  beetles=0,other=0,total=0)
...
# setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "Processing_files/Datasets_storage/field_level_data_Breno_M_Freitas_Anacardium_occidentale_Brazil_2012.csv")
# setwd(dir_ini)
...
data.site <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/field_level_data_Breno_M_Freitas_Anacardium_occidentale_Brazil_2012_corrected.xlsx",
                       sheet = "field_level_data_Breno_M_Freita", startRow = 1)
data.site <- as_tibble(data.site)

# setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(data.site, "Processing_files/Datasets_storage/field_level_data_Breno_M_Freitas_Anacardium_occidentale_Brazil_2012.csv")
# setwd(dir_ini)
