...
##########################
#Data: DAINESE, Bore02: bore02
##########################


data.site <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Bore02_Datacollection_pollination.xlsx",
...
data.site <- data.site %>% select(-'Annual/perennial')%>%
  rename(site_id=SiteID,X_UTM=X,Y_UTM=Y,zone_UTM=UTMzone,sampling_year=Year,crop=Crop.species,
         management=Management) %>% mutate(study_id="Virginie_Boreux_Malus_domestica_Germany_2015")
...
data.Functioning <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Bore02_Datacollection_pollination.xlsx",
...
yield_aux1 <- data.Functioning %>% spread(Exclosure.treatment,Function)

yield_aux1 <- yield_aux1 %>%
  rename(yield_treatments_no_pollinators=sealed,
         yield=open,
         yield_treatments_pollen_supplement=hand,
         yield_units=Type.of.function)

#TO percentage
yield_aux1$yield_treatments_no_pollinators <- 100*yield_aux1$yield_treatments_no_pollinators
yield_aux1$yield <- 100*yield_aux1$yield
yield_aux1$yield_treatments_pollen_supplement <- 100*yield_aux1$yield_treatments_pollen_supplement
yield_aux1$yield_units <- "final fruitset % (100*#fruits/#flowers)"

yield_aux1 <- select(yield_aux1,-`<NA>`)
...
data.LandscapeData <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Bore02_Datacollection_pollination.xlsx",
                                 sheet = "LandscapeData", startRow = 2)

data.LandscapeData <- data.LandscapeData[,1:4]
...
data.site <- data.site %>% mutate(country="Germany",Publication=NA,
                                  Credit="Virginie Boreux (now at Freiburg Uni), ETH Zurich, Project EcoFruit",
...
data.species <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Bore02_Datacollection_pollination.xlsx",
...
data.species_04$Guild[data.species_04$Organism_ID=="Andrena sp. "] <- "other_wild_bees"
data.species_04$Guild[data.species_04$Organism_ID=="Bombus sp. "] <- "bumblebees"
data.species_04$Guild[data.species_04$Organism_ID=="Osmia sp. "] <- "other_wild_bees"

data.species_04 %>% filter(is.na(Guild)) %>% group_by(Organism_ID,Family) %>% count()

data.species_04 <- data.species_04 %>% mutate(total_sampled_area=120,
                                              total_sampled_time=30,
                                              total_sampled_flowers=NA,
                                              Description="30min Observation/site (3x2 transects). a 5min transect (20m) was walked between 2 apple tree rows")


insect_sampling <- tibble(
  study_id = "Virginie_Boreux_Malus_domestica_Germany_2015",
...
write_csv(insect_sampling, "Processing_files/Datasets_storage/insect_sampling_Virginie_Boreux_Malus_domestica_Germany_2015.csv")
...
abundace_field <- data.species_04 %>%
...
if (percentage_species_morphos<0.8){ #NOT ENOUGH SPECIES+MORPHOSP
...
# setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "Processing_files/Datasets_storage/field_level_data_Virginie_Boreux_Malus_domestica_Germany_2015.csv")
# setwd(dir_ini)
