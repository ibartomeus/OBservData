...
options(digits=14)
##########################
#Data: DAINESE, Taki01: Taki01
##########################


data.site <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Taki01_Datacollection_pollination.xlsx",
                          sheet = "SiteData", startRow = 2)
data.site <- as_tibble(data.site)

data.site <- data.site %>% filter(Year==2007)
data.site <- data.site %>%
  separate(col = X, into = c("Dirx", "X"),"(?<=[ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz]) ?(?=[0-9])")
data.site <- data.site %>%
  separate(col = Y, into = c("Diry", "Y"),"(?<=[ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz]) ?(?=[0-9])")
...
data.site <- data.site %>% select(-'Annual/perennial',-Dirx,-Diry)%>%
  rename(site_id=SiteID,longitude=X,latitude=Y,sampling_year=Year,crop=Crop.species,
         management=Management) %>% mutate(study_id="Taki01")
...
data.Functioning <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Taki01_Datacollection_pollination.xlsx",
                                 sheet = "Functioning", startRow = 2)

data.Functioning <- as_tibble(data.Functioning)

data.Functioning <- data.Functioning %>% filter(Year.of.sampling==2007)

data.Functioning <- data.Functioning %>%
  rename(site_id=SiteID,sampling_year=Year.of.sampling)

data.Functioning$Exclosure.treatment[data.Functioning$Exclosure.treatment=="1x1mm mesh"] <- "1x1mm_mesh"

yield_aux <- data.Functioning %>% spread(Exclosure.treatment,Function)

#Adapt units of seed set to percentage
yield_aux$`1x1mm_mesh` <- 100*yield_aux$`1x1mm_mesh`
yield_aux$open <- 100*yield_aux$open

yield_aux <- yield_aux %>%
  rename(yield_treatments_no_pollinators=`1x1mm_mesh`,
         yield=open,
...
data.LandscapeData <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Taki01_Datacollection_pollination.xlsx",
                                 sheet = "LandscapeData", startRow = 2)
data.LandscapeData <- as_tibble(data.LandscapeData[,1:4])
...
data.species <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Taki01_Datacollection_pollination.xlsx",
...
data.species_01$Guild[data.species_01$Organism_ID=="Arge nipponensis "] <- "non_bee_hymenoptera"
data.species_01$Guild[data.species_01$Organism_ID=="Athalia rosae "] <- "non_bee_hymenoptera"
data.species_01$Guild[data.species_01$Organism_ID=="Dolerus similis "] <- "non_bee_hymenoptera"
...
data.species_01 <- data.species_01 %>% mutate(total_sampled_area=NA,
                                              ...
                                              Description="Measures per site: 10 plants, 3 sampling rounds in each year
")


insect_sampling <- tibble(
  study_id = "Hisatomo_Taki_Fagopyrum_esculentum_Japan_2007",
...
write_csv(insect_sampling, "Processing_files/Datasets_storage/insect_sampling_Hisatomo_Taki_Fagopyrum_esculentum_Japan_2007.csv")
...
# setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "Processing_files/Datasets_storage/field_level_data_Hisatomo_Taki_Fagopyrum_esculentum_Japan_2007.csv")
# setwd(dir_ini)
