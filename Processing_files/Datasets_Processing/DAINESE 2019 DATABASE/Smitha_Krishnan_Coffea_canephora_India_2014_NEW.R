...
options(digits=14)
##########################
#Data: DAINESE, krish02
##########################


data.site <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Krish02_DataCollection_Pollination.xlsx",
                          sheet = "SiteData", startRow = 2)
data.site <- as_tibble(data.site)

data.site <- data.site %>% filter(Year==2014)
...
data.site <- data.site %>% select(-'Annual/perennial')%>%
  rename(site_id=SiteID,latitude=Y,longitude=X,sampling_year=Year,crop=Crop.species,
         management=Management) %>% mutate(study_id="krish011")
...
#distance 3 is selected because it appears in every (25) site.

data.site <- data.site %>% filter(distance==3) %>% select(-distance)
...
sampling_year <- sampling %>% filter(sampling_year==2014) %>%
...
#####################################
# VISITATION RATE
# Note that there is only one treatment: open

data.Final <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Krish02_Datacollection_pollination.xlsx",
                                 sheet = "Final Ecosystem Services", startRow = 2)

data.Final <- as_tibble(data.Final)

data.Final <- data.Final %>% filter(Year.of.sampling==2014)
...
# Crop.yield.(dried.and.hulled.beans)

yield_aux1 <- data.Final %>% select(site_id,sampling_year,`Crop.yield.(dried.and.hulled.beans)`,Unit)

yield_aux1$Unit <- paste("kg/ha (dried and hulled beans)")

yield_aux1 <- yield_aux1 %>%
  rename(yield=`Crop.yield.(dried.and.hulled.beans)`,
         yield_units=Unit)
...
# `Crop.yield.(dried.berries)`

yield_aux2 <- data.Final %>% select(site_id,sampling_year,`Crop.yield.(dried.berries)`,Unit)

yield_aux2$Unit <- paste("kg/ha (dried berries)")

yield_aux2 <- yield_aux2 %>%
  rename(yield2=`Crop.yield.(dried.berries)`,
         yield2_units=Unit)
...
data.site <- data.site %>% mutate(country="India",Publication=NA,
                                  Credit="Smitha Krishnan, ETH Zurich and Ms. Maike Nesper (The PI of the project MOCA is Prof. Jaboury Ghazoul)",
                                  email="smithakrishnan@gmail.com / S.Krishnan@cgiar.org / maike.nesper@gmail.com / mnesper@ethz.ch")

################################
#COLLECTING INSECT SAMPLING DATA
################################

data.species <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Krish02_Datacollection_pollination.xlsx",
...
#reported abundances are the TOTAL of three observations
data.species_01$abundance <- round(data.species_01$abundance)


data.species_01 <- data.species_01 %>% mutate(total_sampled_area=NA,
                                              total_sampled_time=45,
                                              total_sampled_flowers=NA,
                                              Description="The abundance is the total abundance of replications of 15 minute observations on each of the 3 plants observed (7 branches/plant with 4 floral clusters/branch)")



insect_sampling <- tibble(
  study_id = "Smitha_Krishnan_Coffea_canephora_India_2014",
...
write_csv(insect_sampling, "Processing_files/Datasets_storage/insect_sampling_Smitha_Krishnan_Coffea_canephora_India_2014.csv")
...
# There are "beetles","honeybees","lepidoptera","non_bee_hymenoptera", "other",
...
number_sites <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Krish02_DataCollection_Pollination.xlsx",
                       sheet = "SiteData", startRow = 2)

number_sites <- as_tibble(number_sites)

number_sites <- number_sites %>% filter(Year==2014)
...
#total time observation per site = number of sites *3*15=45*n
#total number of inflorescences = 3*n*7*4=84n

for (i in 1:nrow(visit_aux)){
  a <- 45*84*pull(visit_aux[i,2])*pull(visit_aux[i,2])
...
field_level_data <- tibble(
  ...
  total_sampled_time=45*data.site$n, #45 de observaci?n en cada punto de observaci?n de cada granja
...
# setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "Processing_files/Datasets_storage/field_level_data_Smitha_Krishnan_Coffea_canephora_India_2014.csv")
# setwd(dir_ini)
