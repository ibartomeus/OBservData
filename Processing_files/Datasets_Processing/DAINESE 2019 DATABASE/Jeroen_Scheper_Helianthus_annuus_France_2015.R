...
options(digits=14)
##########################
#Data: DAINESE, Sche01: sche01
##########################


data.site <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Sche01_Datacollection_pollination.xlsx",
                          sheet = "SiteData", startRow = 2)
data.site <- as_tibble(data.site)

data.site$study_id <- "Jeroen_Scheper_Helianthus_annuus_France_2015"
...
sputm <- SpatialPoints(data.site[,2:3], proj4string=CRS("+proj=utm +zone=30 +datum=WGS84"))
...
data.Functioning <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Sche01_Datacollection_pollination.xlsx",
...
data.LandscapeData <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Sche01_Datacollection_pollination.xlsx",
...
data.site <- data.site %>% mutate(country="France",Publication=NA,
                                  Credit="Jeroen Scheper (WUR), David Kleijn (WUR), Isabelle Badenhausser (CEBC-CNRS), Vincent Bretagnolle (CEBC-CNRS), Ignasi Bartomeus (EBD-CSIC), Carlos Zaragoza-Trello (EBD-CSIC) ",
...
data.species <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/Sche01_Datacollection_pollination.xlsx",
...
percentage_species_morphos <-
  sum(data.species_01$Identified.to %in% c("species","morphospecies","species complex"))/nrow(data.species_01)
...
data.species_01$Guild[data.species_01$Organism_ID=="Andrena flavipes"] <- "other_wild_bees"


data.species_01 %>% filter(is.na(Guild)) %>% group_by(Organism_ID,Family) %>% count()

data.species_01 <- data.species_01 %>% mutate(total_sampled_area=4*150,
                                              total_sampled_time=NA,
                                              total_sampled_flowers=NA,
                                              Description="Measures per site: per site 2 x 150m2 transects (one edge, one centre), per site 10 x plant pairs from edge to centre of field (5m intervals between pairs). Temporal replicates:	two sampling rounds for transect surveys")


insect_sampling <- tibble(
  study_id = "Jeroen_Scheper_Helianthus_annuus_France_2015",
...
write_csv(insect_sampling, "Processing_files/Datasets_storage/insect_sampling_Jeroen_Scheper_Helianthus_annuus_France_2015.csv")
...
abundance_aux <- abundance_aux %>% mutate(other_flies=0,beetles=0,humbleflies=0,
                                          lepidoptera=0,non_bee_hymenoptera=0,other=0,total=0)
...
field_level_data_mod <- read_csv("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/field_level_data_Jeroen_Scheper_Helianthus_annuus_France_2015.csv")
...
# setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "Processing_files/Datasets_storage/field_level_data_Jeroen_Scheper_Helianthus_annuus_France_2015.csv")
# setwd(dir_ini)
