...
data.site <- data.site %>% mutate(country="Brazil",Publication="10.1126/science.aac7287",
...
percentage_species_morphos <-
  sum(data.species_03$Identified.to %in% c("species","morphospecies"))/nrow(data.species_03)
...
data.species_03$Guild[data.species_03$Organism_ID=="Apoica pallens "] <- "non_bee_hymenoptera"
data.species_03$Guild[data.species_03$Organism_ID=="Brachygastra lecheguana "] <- "non_bee_hymenoptera"
data.species_03$Guild[data.species_03$Organism_ID=="Brachygastra lecheguana "] <- "non_bee_hymenoptera"
data.species_03$Guild[data.species_03$Organism_ID=="Centris  byrsonimae "] <- "other_wild_bees"
data.species_03$Guild[data.species_03$Organism_ID=="Centris byrsonimae "] <- "other_wild_bees"
data.species_03$Guild[data.species_03$Organism_ID=="Stratiomyidae sp.2 "] <- "syrphids"
data.species_03$Guild[data.species_03$Organism_ID=="Xylocopa cearensis  "] <- "other_wild_bees"
...
data.species_03 <- data.species_03 %>% mutate(total_sampled_area=Number.of.censuses*150,
                                              ...
                                              Description="6 subplots per site,	150 m, 10 sampling rounds in one season (6 censuses reported)")


insect_sampling <- tibble(
  study_id = "Breno_M_Freitas_Anacardium_occidentale_Brazil_2011",
...
write_csv(insect_sampling, "Processing_files/Datasets_storage/insect_sampling_Breno_M_Freitas_Anacardium_occidentale_Brazil_2011.csv")
...
# "other"               "other_flies"         "other_wild_bees"     "syrphids"

# GUILDS:honeybees, bumblebees, other wild bees, syrphids, humbleflies,
# other flies, beetles, non-bee hymenoptera, lepidoptera, and other

abundance_aux <- abundance_aux %>% mutate(beetles=0,bumblebees=0,
                                          humbleflies=0,
                                          total=0)
...
# setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "Processing_files/Datasets_storage/field_level_data_Breno_M_Freitas_Anacardium_occidentale_Brazil_2011.csv")
# setwd(dir_ini)
...
data.site <- read.xlsx("Processing_files/Datasets_processing/DAINESE 2019 DATABASE/DATASETS/field_level_data_Breno_M_Freitas_Anacardium_occidentale_Brazil_2011_corrected.xlsx",
                       sheet = "field_level_data_Breno_M_Freita", startRow = 1)
data.site <- as_tibble(data.site)


# setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(data.site, "Processing_files/Datasets_storage/field_level_data_Breno_M_Freitas_Anacardium_occidentale_Brazil_2011.csv")
# setwd(dir_ini)
