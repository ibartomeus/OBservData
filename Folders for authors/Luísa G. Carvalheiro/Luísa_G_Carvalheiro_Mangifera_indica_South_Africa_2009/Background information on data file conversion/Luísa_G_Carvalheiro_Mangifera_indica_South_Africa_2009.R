...
data.site <- data.site %>% filter(StudyID=="carv03")
...
data.Functioning <- data.Functioning %>% filter(grepl("carv03",data.Functioning$SiteID,ignore.case = TRUE))

data.Functioning$Exclosure.treatment<- "NO"

# Rows are repeated
data.Functioning <- unique(data.Functioning)
...
data.Functioning <- data.Functioning %>% select(-Notes) %>%
  rename(site_id=SiteID,sampling_year=Year.of.sampling,yield=NO,
...
data.Final <- data.Final %>% filter(grepl("carv03",data.Final$SiteID,ignore.case = TRUE))

data.Final <- data.Final %>%
  select(SiteID,`Crop.yield.(exact.match.OR.adjacent.plot.(each.plot.has.ca.1ha)`,Unit) %>%
  rename(site_id=SiteID,yield2=`Crop.yield.(exact.match.OR.adjacent.plot.(each.plot.has.ca.1ha)`,
         yield2_units=Unit)
...
data.site <- data.site %>% mutate(country="South Africa",Publication="Carvalheiro et al 2012 Journal of Applied Ecology",
...
tax_res <- read_csv("taxon_table_carv03.csv") %>% rename(Organism_ID=pollinator)
...
insect_sampling <- tibble(
  study_id = "Luísa_G_Carvalheiro_Mangifera_indica_South_Africa_2009",
...
write_csv(insect_sampling, "insect_sampling_Luísa_G_Carvalheiro_Mangifera_indica_South_Africa_2009.csv")
...
abundance_aux <- abundance_aux %>% mutate(bumblebees=0,lepidoptera=0,humbleflies=0,total=0)
...
# NOTE THAT THESE RESULTS ARE NOT VISITATION RATES!!!
...
setwd("C:/Users/USUARIO/Desktop/OBservData/Datasets_storage")
write_csv(field_level_data, "field_level_data_Luísa_G_Carvalheiro_Mangifera_indica_South_Africa_2009.csv")
setwd(dir_ini)
